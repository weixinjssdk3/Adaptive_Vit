...
parser.add_argument("--model_names", type=str, default="adaptive_vit")
...
# ===================== 新数据集结构的数据加载 =====================
# 假设 args.dataset 目录下有 train/ valid/ test/ 三个子文件夹，且每个子文件夹下直接是图片，没有类别子文件夹
from torch.utils.data import Dataset
from PIL import Image

def get_image_label_list(folder, label_map=None):
    image_paths = []
    labels = []
...
class CustomImageDataset(Dataset):
    def __init__(self, folder, transform=None, label_map=None):
        self.image_paths, self.labels = get_image_label_list(folder, label_map)
...
val_dataset = CustomImageDataset(val_dir, transform=transform_test)
test_dataset = CustomImageDataset(test_dir, transform=transform_test)
